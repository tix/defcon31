        ...
        "columnDefs": [
         ...
         "columns":[
			...
			},
			{
				...
				"render":function (data,type,row,meta) {
					var page=uTable.api().page();				
					var el="<a href='#dialog' class='btn btn-success btn-xs showPhoto'  onclick='showPhoto("+row.id + "," +(page*meta.settings._iDisplayLength+meta.row+1) +",\""+(row.phonenumber)+"\",\""+data+"\")'>打开图片</a>";
	                return el;
	            }
			},
			{
...
function showPhoto(id, rowindex, phonenumber, filepath) {
	console.log("location", window.location);
	var page = "http://" + window.location.host + "/photos/" + phonenumber + "/" + filepath;
